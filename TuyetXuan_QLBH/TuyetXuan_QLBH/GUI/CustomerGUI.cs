...
namespace TuyetXuan_QLBH.GUI
{
    public partial class CustomerGUI : Form
    {
        ...
        public CustomerGUI()
        {
            InitializeComponent();//contractor
        }
        ...
        private void btNew_Click(object sender, EventArgs e)
        {
            ...
            int ma;
            if (!int.TryParse(tbMa.Text, out ma) || ma <= 0) // Add the condition to check for non-positive IDs
            {
                MessageBox.Show("Mã phải là số nguyên dương!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (!System.Text.RegularExpressions.Regex.IsMatch(diachi, @"^[\w\d\s/]+$"))
            {
                MessageBox.Show("Địa chỉ không hợp lệ! Địa chỉ có thể chứa chữ, số, dấu cách và dấu gạch chéo.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
